import random
import math
from Classes.Conjunto import *
from Classes.Ponto import *
from Voronoi.Datatype import *


# Source: (C++) http://www.cs.hmc.edu/~mbrubeck/voronoi.html

class Voronoi:
    def __init__(self, conjuntoPonto: Conjunto):
        #lista = conjuntoPonto.RetornaConjuntoEmLista()
        point = conjuntoPonto.RetornaConjuntoEmLista()
        #for i in range(len(lista)):
        #    points.append((lista[i].getX(),))
        ...
        for i in range(len(point)):
            self.points.push(point[i])
            # keep track of bounding box size
            if point[i].x < self.x0: self.x0 = point[i].x
            if point[i].y < self.y0: self.y0 = point[i].y
            if point[i].x > self.x1: self.x1 = point[i].x
            if point[i].y > self.y1: self.y1 = point[i].y
