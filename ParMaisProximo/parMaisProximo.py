...
# closest pairs by divide and conquer adapted
# David Eppstein, UC Irvine, 7 Mar 2002

def closestpair(pontoconjunto : Conjunto):
    def square(x):
        return x * x
    ...
    for i in range(len(Lista)):
        L.append((Lista[i].getX(), Lista[i].getY()))
...
def desenhaPontos(tupla, pontos : Conjunto):
    lista = pontos.RetornaConjuntoEmLista()
    for i in range(len(lista)):
    ...
    ay2 = tupla[1][1]
    plt.plot([ax1, ax2],
             [ay1, ay2], zorder=2, color='red')
    plt.ylabel('Par mais próximo')
    plt.show()
